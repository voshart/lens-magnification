...
Manufacturer image-quality claims and distance ranges are provenance only. The current simulator is centered on each setup's maximum-magnification configuration rather than the full focusing range. Published infinity-focus distances, manufacturer working-distance ranges, focus conditions, and the exact distance terminology used by each manufacturer are preserved in `closeup-reference-data.js` for possible future use. When the host lens has a defensible native close-focus working distance, the current setup instead shows a first-order maximum-magnification WD estimate from that native WD and the selected diopter power.

## Accuracy

Results are planning estimates rather than measurements of a physical setup. Some calculations use thin-lens approximations, nominal focal lengths, published manufacturer specifications, or assumed pupil magnification. Close-up-lens power is modeled independently of optical quality; aberrations, multi-element correction, spacing, and host-lens interactions are not simulated. Generic close-up presets represent nominal optical power only, so construction, correction, coatings, power tolerance, and image quality remain unknown. Close-up-lens working distance is estimated only when the host lens has a defensible native WD, using a first-order thin/in-contact approximation at the host lens's native maximum-magnification setting; real attachment thickness, spacing, principal planes, and internal focusing can shift the result. Manufacturer construction and distance guidance remain reference metadata rather than calculated performance. The interface displays additional warnings when a value is unavailable or especially uncertain.
